...
class SqlPager /*extends db_pager */ implements PagerInterface
{
    var $sql;
    var $name;
    ...
    function set_where($where = null)
    {
        if ($where) {
            ...
            if (
                count($where) == count($this->extra_where) &&
    ...
    function _sql_gen($count = false)
    {
        ...
        if ($this->extra_where && count($this->extra_where) > 0) {
            $where .= ($where == '' ? '' : ' AND ')
    ...
    static function &new_db_pager($name, $sql, $coldef, $table = null, $key = null, $page_len = 0)
    {
        ...
        if (!isset($_SESSION[$name])) {
            $_SESSION[$name] = new SqlPager($sql, $name, $table, $page_len);
